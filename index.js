...
const port = process.env.PORT || 5501; // Change to a different port
 // Set the port to 5501 or environment variable
...
async function verifyUser(req, res, next) {
  const username = req.body.username || req.params.username; // Handle both body and URL params
  ...
  try {
      ...
      console.log("User found in player collection:", user);

      // If not found, check the `admin` collection
      if (!user) {
...
app.delete('/deleteLevel/:username', verifyUser, async (req, res) => {
  try {
    ...
    console.error('Error deleting game level:', err); // Log any errors during game level deletion
    res.status(500).json({ error: 'Internal server error' }); // Send error response
  }
});
